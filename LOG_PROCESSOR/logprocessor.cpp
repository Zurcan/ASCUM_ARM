...
QString logProcessor::getLogFileName()
{
    return FileName;
}
...
bool logProcessor::readTimeInversionFlag()
{
    if(segmentHeader.flags&&0x00000001)
        return true;
    else return false;
}

long logProcessor::getSegmentFlags()
{
    return segmentFlags;
}

bool logProcessor::readRecordByNumber(int recCount, int Size, int savedDataPointer,int recNum)
{

    clearBuff();
    int tmp1,tmp2;
    logRecordPointer = logDataPointer;
    logRecordPointer = Size*recNum + savedDataPointer;
    tmpFile.seek(logDataPointer);
    tmp1 = tmpFile.bytesAvailable();
    if(tmp1>=Size)
    {
        if(!tmpFile.seek(logRecordPointer))
            return false;
        //tmp1 = tmpFile.bytesAvailable();
        if(logRecordPointer<=Size*recCount+savedDataPointer)
        {
                                                         ...
                                                         //the next step is to get selected segment's Header and set pointer to it's first record;
{
    ...
    pointFileValAtLDPtr(tmpDataPointer);
        ...
        pointFileValAtLDPtr(tmpDataPointer);
        if(ID==segmentHeader.LID)
        {
            segmentFlags = segmentHeader.flags;
            return 0;
        }
        else
            {
...
int logProcessor::getSegmentsCount()
{
    int retval=0;
    setValueLDPtr(0);
    long idarr[7];
    idarr[0] = stgid_Language;
    idarr[1] = stgid_ShortLog;
    idarr[2] = stgid_StatLog;
    idarr[3] = 0x00000081;//camera log
    idarr[4] = stgid_ShortLog|0x80000000;
    idarr[5] = stgid_StatLog|0x80000000;
    idarr[6] = 0x00000081|0x80000000;
    for(int i = 0; i < 7; i++)
    {
        if(!selectSegment(idarr[i]))
            retval++;
